...
### 11.1 Unit tests (JUnit 5, pure JVM — no Minecraft)

- `SeasonState` phase transitions: `CREATED → RUNNING → ENDED` are valid; any other transition throws.
- `Faction` accessor math: `memberCount` reflects `members.size`; `addSkillpoints` / `purchaseSkill` update pool and map.
- `PantheonSavedData` NBT round-trip: create → mutate → `save` to `CompoundTag` → `load` → assert equal. Uses `HolderLookup.Provider` stub if needed, or tests the NBT-encoding helpers directly.

### 11.2 NeoForge GameTests (`@GameTest`, server-side)
...
GameTests run via `./gradlew gameTestServer` or the `runGameTestServer` config already in `build.gradle`.

### 11.3 Manual client test
